...
Partial Public Class pagePengembalianMobil
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            myCon = New OleDbConnection(strCon)
            myCon.Open()
            strSQL = "SELECT * FROM TBL_PENGEMBALIAN WHERE NAMA_CUSTOMER = '" & txtNamaCustomer.Text & "' AND KD_MOBIL = '" & strKodeMobil & "'"
            objCommand = New OleDbCommand(strSQL, myCon)
            objReaderExists = objCommand.ExecuteReader()
            If objReaderExists.HasRows Then
                objReaderExists.Close()
                'Update
                strSQL = "UPDATE TBL_PENGEMBALIAN SET TGL_PENGEMBALIAN = '" & dtTgl.SelectedDate & "',QTY = " & CInt(txtQty.Text) & " WHERE NAMA_CUSTOMER = '" & txtNamaCustomer.Text & "' AND KD_MOBIL = '" & strKodeMobil & "'"
                objCommand = New OleDbCommand(strSQL, myCon)
                If objCommand.ExecuteNonQuery() Then
                    ...
                    If objReader.HasRows Then
                        ...
                        strSQL = "UPDATE TBL_STOCK SET QTY_IN = " & dblQtyIn & ", QTY_AKHIR = " & dblQtyAkhir & " WHERE KD_MOBIL = '" & strKodeMobil & "'"
                        objCommand = New OleDbCommand(strSQL, myCon)
            ...
            Else
                objReaderExists.Close()
                'Insert
                strSQL = "INSERT INTO TBL_PENGEMBALIAN (NAMA_CUSTOMER,TGL_PENGEMBALIAN,KD_MOBIL,QTY) VALUES ('" & txtNamaCustomer.Text & "','" & dtTgl.SelectedDate & "','" & strKodeMobil & "'," & txtQty.Text & ")"
                objCommand = New OleDbCommand(strSQL, myCon)
                ...
                If objCommand.ExecuteNonQuery() Then
                    ...
                    If objReader.HasRows Then
                        ...
                        objCommand.ExecuteNonQuery()
                    Else
                        strSQL = "INSERT INTO TBL_STOCK (KD_MOBIL,QTY_IN,QTY_OUT,QTY_AKHIR,QTY_ADD) VALUES('" & strKodeMobil & "'," & txtQty.Text & ",0," & txtQty.Text & "," & txtQty.Text & ")"
    ...
    Private Sub ListGrid()
        Try
            myCon = New OleDbConnection(strCon)
            myCon.Open()
            strSQL = "SELECT DISTINCT B.NAMA_CUSTOMER,B.TGL_PENGEMBALIAN,B.KD_MOBIL,A.NM_MOBIL,a.HRG_SEWA,B.QTY,(a.HRG_SEWA * B.QTY) AS TOT_HRG FROM TBL_MOBIL A, TBL_PENGEMBALIAN B WHERE A.KD_MOBIL = B.KD_MOBIL ORDER BY B.NAMA_CUSTOMER,B.KD_MOBIL ASC"
    ...
    Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        Try
            ...
            strSQL = "DELETE FROM TBL_PENGEMBALIAN WHERE NAMA_CUSTOMER = '" & txtNamaCustomer.Text & "' AND KD_MOBIL = '" & strKodeMobil & "'"
            objCommand = New OleDbCommand(strSQL, myCon)
            If objCommand.ExecuteNonQuery Then
                strSQL = "SELECT * FROM TBL_STOCK WHERE KD_MOBIL = '" & strKodeMobil & "'"
                objCommand = New OleDbCommand(strSQL, myCon)
